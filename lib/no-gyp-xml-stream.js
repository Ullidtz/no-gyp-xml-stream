...
NoGypXmlStream.getChildrenFromPath = function(path, childCallBack, options, parentCallBack){
    let readStream = fs.createReadStream(path);
    NoGypXmlStream.getChildrenFromReadStream(readStream, childCallBack, options, parentCallBack);
};

NoGypXmlStream.getChildrenFromReadStream = function(readStream, childCallBack, options, parentCallBack){
    let saxStream = sax.createStream(true, { strictEntities: false });
    readStream.pipe(saxStream);
    NoGypXmlStream.getChildrenFromSaxStream(saxStream, childCallBack, options, parentCallBack);
};

NoGypXmlStream.getChildrenFromSaxStream = function(saxStream, childCallBack, options, parentCallBack){
    let parentName = 'xml';
    let elementStack = [];
    ...
    saxStream.on("opentag", function (node) {
        if(node.name.toLowerCase() == parentName){
            parentElement = {};
            parentElement[opts.attributesVariableName] = node.attributes;
            return;
        }
        if(childCount === 0) {
            childNameFilter = node.name.toLowerCase();
            elementStack = [];
            let newElem = {};
    ...
    // proceses character data (cdata)
    saxStream.on("cdata", function (txt) {
        if (!NoGypXmlStream.isNullOrWhitespace(txt)) {
            var elemc = elementStack.pop();
            if(!elemc[opts.textVariableName])elemc[opts.textVariableName] = "";
            elemc[opts.textVariableName] += txt.trim();
...
exports.getXmlStreamChildren = function(path, childCallBack, options, parentCallBack){
    return NoGypXmlStream.getChildrenFromPath(path, childCallBack, options, parentCallBack);
};
